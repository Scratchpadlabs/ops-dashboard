        ...
        <thead class="sticky top-0 z-20">
          <tr>
            ...
            <th
              ...
              class="bg-slate-50 border-b border-slate-200 px-1.5 py-2 text-center cursor-pointer hover:bg-slate-100 transition-colors relative group"
              style="min-width:74px; max-width:96px"
              v-tooltip.bottom="headerTooltip(s)"
              @click="$emit('select-column', s.id, $event)"
            >
              <button
                type="button" class="absolute top-0.5 right-0.5 text-slate-300 hover:text-violet-600 opacity-0 group-hover:opacity-100 transition-opacity"
                title="View / edit survey details"
                @click.stop="$emit('view-survey', s.id)"
              >
...
const emit = defineEmits([
  ...
  'select-cells', 'select-row', 'select-column', 'toggle-expand', 'view-survey',
])
