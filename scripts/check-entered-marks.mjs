 ...
 * marks entered, for a given school+term. Same data SheetsStatusTab.vue
 * shows in the UI (lastEditedAt/lastEditedBy/entryCount per classId+
 * subjectId), surfaced as a script so it can be checked before running a
 * destructive fix like delete-superseded-computer-assessments.mjs.
 *
...
if (!schoolId || !termId) {
  console.error('Missing required --school=<id> and --term=<id>')
  process.exit(1)
}
...
async function main() {
  const snap = await db.collection('schools').doc(schoolId).collection('smart_sheet_entries')
    .where('termId', '==', termId).get()
  ...
  rows.sort((a, b) => a.subjectId.localeCompare(b.subjectId) || a.classId.localeCompare(b.classId))
  const toPrint = onlyEntered ? rows.filter(r => r.entryCount > 0) : rows

  const header = 'classId,subjectId,entryCount,lastEditedAt,lastEditedBy,isFrozen,sheetId\n'
  const body = toPrint.map(r => `${r.classId},${r.subjectId},${r.entryCount},${r.lastEditedAt},"${r.lastEditedBy}",${r.isFrozen},${r.sheetId}`).join('\n')
  writeFileSync(outPath, header + body + '\n')

  const withEntries = rows.filter(r => r.entryCount > 0)
  console.log(`Wrote CSV: ${outPath}`)
  console.log(`${sheets.length} sheet(s) found for term=${termId}${subjectFilter.size ? ` (filtered to ${[...subjectFilter].join(', ')})` : ''}.`)
  console.log(`${withEntries.length} sheet(s) have at least one mark entered:`)
  for (const r of withEntries) {
    console.log(`  ${r.subjectId} / ${r.classId} — ${r.entryCount} entries, last edited ${r.lastEditedAt || 'unknown'} by ${r.lastEditedBy || 'unknown'}${r.isFrozen ? ' [FROZEN]' : ''}`)
  }
}
