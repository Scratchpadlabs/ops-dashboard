...
# Lightweight header-only alias dicts for teachers/subjects — used exclusively
# by tabular_parser.detect_entity_type() (email intake's DETECTION mode) to
# guess which entity a spreadsheet attachment holds by header score alone.
# Deliberately NOT wired into parse_tabular_file() for full row extraction:
# main.py's SCHEMAS/_process_one_file still routes teachers/subjects through
# the LLM path exclusively — only students has a deterministic row parser.
# Keep the field names themselves in sync with main.py's SCHEMAS row keys.
TEACHER_HEADER_ALIASES = {
    "teacher_name": ["teacher name", "teachers name", "name of teacher", "name", "faculty name"],
    "email": ["email", "e-mail", "email id", "email address"],
    "class_teacher_of": ["class teacher of", "class teacher", "ct of"],
    "subject": ["subject", "subjects", "subject taught"],
    "grade": ["class", "grade", "std", "standard"],
    "section": ["sec", "section", "div", "division"],
}
TEACHER_REQUIRED_FIELD = "teacher_name"

SUBJECT_HEADER_ALIASES = {
    "stream": ["stream", "board"],
    "grade_band": ["grade band", "class", "grade", "std", "standard"],
    "subject": ["subject", "subjects", "subject name"],
    "area": ["area", "scholastic area", "type"],
}
SUBJECT_REQUIRED_FIELD = "subject"
