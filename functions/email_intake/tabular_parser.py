...
from normalize import (
    ...
    TEACHER_HEADER_ALIASES, TEACHER_REQUIRED_FIELD,
    SUBJECT_HEADER_ALIASES, SUBJECT_REQUIRED_FIELD,
)
...
# ------------------------------------------------------- entity detection --
# Email intake's "DETECTION mode" (see functions/email_intake/main.py): a
# cheap, LLM-free header-only classification of a spreadsheet-like attachment
# as students/teachers/subjects/unknown, so an auto-stage decision can be made
# without ever parsing rows for the two entities (teachers/subjects) that
# still only have an LLM extraction path in main.py's SCHEMAS. Never raises —
# same never-crash contract as parse_tabular_file(); an unreadable/empty file
# just classifies as unknown.
_DETECTION_CANDIDATES = (
    ("students", STUDENT_HEADER_ALIASES, STUDENT_REQUIRED_FIELD),
    ("teachers", TEACHER_HEADER_ALIASES, TEACHER_REQUIRED_FIELD),
    ("subjects", SUBJECT_HEADER_ALIASES, SUBJECT_REQUIRED_FIELD),
)


def detect_entity_type(filename, raw, min_header_score=2):
    """Returns (entity_or_None, score, sheet_name_or_None). `entity` is None
    ("unknown") when no candidate's required field was found in any sheet's
    header, or the best-scoring candidate is below min_header_score. Ties
    across entities keep whichever was tried first in _DETECTION_CANDIDATES
    (students first — the far more common attachment type)."""
    try:
        ftype = sniff_file_type(raw)
        if ftype == "empty":
            return None, 0, None
        if ftype == "xlsx":
            sheets, _ = _load_xlsx(raw)
        elif ftype == "ole":
            sheets, _ = _load_ole(raw)
        elif ftype == "html":
            sheets, _ = _load_html(raw)
        else:
            sheets, _, _ = _load_text(raw)
    except Exception:
        return None, 0, None

    best_entity, best_score, best_sheet = None, 0, None
    for entity, aliases, required_field in _DETECTION_CANDIDATES:
        alias_lookup = build_alias_lookup(aliases)
        for sheet_name, sheet_rows, _hidden in sheets:
            if _is_sheet_effectively_empty(sheet_rows):
                continue
            try:
                found = _detect_header(sheet_rows, alias_lookup, required_field, min_header_score)
            except Exception:
                continue
            if found is not None and found[0] > best_score:
                best_score, best_entity, best_sheet = found[0], entity, sheet_name

    return best_entity, best_score, best_sheet
